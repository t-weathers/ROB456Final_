...
class RVizAStar(object):
        ...
        if self.driving:
            ...
            if not self.pre_orientation_completed and self.traj_generated:
                ...
                if abs(self.robot_map_pos_z_ori - ang_goal) < 0.1: #within the goal thres
                    ...
                    self.robot_move_start_time = rospy.get_rostime()
                command.linear.x = 0
                ...
                curr_time = rospy.get_rostime() - self.robot_move_start_time
                robot_pos_tuple = (self.robot_map_pos.x,self.robot_map_pos.y)

                feed_forward_x = self.lin_x_feedfwd(curr_time)
                feedback_x = kp_x * self.lin_err(curr_time, robot_pos_tuple, self.robot_map_pos_z_ori)

                feed_forward_z = self.ang_z_feedfwd(curr_time)
                feedback_z = kp_ang * self.ang_err(curr_time, robot_pos_tuple, self.robot_map_pos_z_ori)
    ...
    def traj_generate(self,map_path):
        ...
        lin_spacer = .04
        self.lin_err = lambda t, robot_pos,robot_dir: np.dot((np.cos(robot_dir),np.sin(robot_dir)),(self.expected_point(t)-robot_pos))
        self.ang_err = lambda t, robot_pos,robot_dir: robot_dir - math.atan2(self.expected_point(t)[1] - robot_dir[1], \
                                                                             self.expected_point(t)[0] - robot_dir[0])
